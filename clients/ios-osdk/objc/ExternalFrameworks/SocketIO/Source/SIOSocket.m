...
- (void)emit:(NSString *)event args:(SIOParameterArray *)args {
    ...
    for (id arg in args) {
        if ([arg isKindOfClass: [NSNull class]]) {
            ...
            NSString *temp = [[NSString alloc] initWithData: [NSJSONSerialization dataWithJSONObject: arg options: 0 error: nil] encoding: NSUTF8StringEncoding];
            NSLog(@" ");
            NSLog(@"+++++++++++++++++++++++++");
            NSLog(@" ");
            NSLog(@"Serialized data : %@",[arg description]);
            NSLog(@" ");
            NSLog(@"+++++++++++++++++++++++++");
            NSLog(@" ");
