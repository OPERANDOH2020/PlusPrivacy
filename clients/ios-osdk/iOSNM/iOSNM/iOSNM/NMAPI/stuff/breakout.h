...
struct arch {
    ...
    struct ar_hdr  toc_ar_hdr;	/* the archive header for this member */
     char      toc_long_name;/* use the long name in the output */
    ...
    /* don't update LC_ID_DYLIB timestamp */
     char dont_update_LC_ID_DYLIB_timestamp;
};
...
struct member {
    ...
    uint32_t      member_name_size; /* the size of the member name */
     char     member_long_name; /* use the extended format #1 for the
...
__private_extern__ struct ofile * breakout(
    ...
    struct arch **archs,
    uint32_t *narchs,
     char calculate_input_prebind_cksum);
