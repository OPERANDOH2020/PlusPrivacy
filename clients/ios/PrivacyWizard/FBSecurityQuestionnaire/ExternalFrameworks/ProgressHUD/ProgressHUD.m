...
#define HUD_Spinner_Size 36.0f

@interface ProgressHUD () {
    bool shouldDimissProgressHud;
}
@end
...
{
    [self shared].interaction = NO;
    [[self shared] hudMake:status image:nil spin:YES hide:YES];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    if (hide) {
        ...
        NSTimeInterval delay = length * 0.04 + 0.5;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    if (hud == nil)
    {
        hud = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleDark]];
    ...
    if (spinner == nil)
    {
        spinner = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"icon_loading_"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]];
        spinner.frame = CGRectMake(0.0, 0.0, HUD_Spinner_Size, HUD_Spinner_Size);
        spinner.tintColor = HUD_SPINNER_COLOR;
    ...
    if (image == nil)
    {
        image = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, HUD_Spinner_Size, HUD_Spinner_Size)];
        image.tintColor = HUD_Error_COLOR;
    }
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    if (label.text != nil)
    {
        ...
        hudWidth = ceilf(labelRect.size.width) + 24;
        hudHeight = ceilf(labelRect.size.height) + 80;
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    label.frame = CGRectMake(labelRect.origin.x, labelRect.origin.y, ceilf(labelRect.size.width), ceilf(labelRect.size.height + 1));
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    {
        hud.alpha = 0;
        hud.transform = CGAffineTransformScale(hud.transform, 1.3, 1.3);
        
        NSUInteger options = UIViewAnimationOptionAllowUserInteraction | UIViewAnimationCurveEaseOut;
        
        [UIView animateWithDuration:0.2 delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:0.5 options:options animations:^{
            hud.transform = CGAffineTransformScale(hud.transform, 1/1.3, 1/1.3);
            hud.alpha = 1;
            self.alpha = 1;
